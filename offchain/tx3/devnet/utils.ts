...
const GENESIS_FILES = ["byron.json", "shelley.json", "alonzo.json", "conway.json"] as const;
...
function shelleySlotConfig(): { zeroTimeMs: number; slotLengthMs: number } {
    const genesis = JSON.parse(readFileSync(join(GENESIS_DIR, "shelley.json"), "utf8")) as {
        systemStart?: string;
        slotLength?: number;
    };
    const zeroTimeMs = Date.parse(genesis.systemStart ?? "");
    if (Number.isNaN(zeroTimeMs)) throw new Error("shelley genesis has no valid systemStart");
...
function deriveTxid(name: string): Uint8Array {
    return blake2b(new TextEncoder().encode(`tx3-test-devnet:utxo:${name}`), { dkLen: 32 });
}
...
function enterpriseAddress(publicKey: Uint8Array): {
    ...
    return { bytes, bech32: bech32.encode(hrp, bech32.toWords(bytes), 1023), keyHash: toHex(keyHash) };
}

/** Big-endian byte expansion of `n` into `len` bytes. */
function beBytes(n: bigint, len: number): number[] {
    const out: number[] = [];
    for (let i = len - 1; i >= 0; i--) out.push(Number((n >> BigInt(i * 8)) & 0xffn));
    return out;
}
...
export class TestDevnet {
    ...
    static async start(options: DevnetOptions): Promise<TestDevnet> {
        ...
        const [trpPort, minibfPort, grpcPort] = await Promise.all([freePort(), freePort(), freePort()]);
        ...
        for (const f of GENESIS_FILES) copyFileSync(join(GENESIS_DIR, f), join(workdir, f));
        writeFileSync(
        ...
        const devnet = new TestDevnet(trpUrl, minibfUrl, proc, workdir, walletsByName, slotConfig);
        ...
        for (;;) {
            ...
            if (Date.now() > deadline) {
                devnet.stop();
                throw new Error(`dolos did not start serving within the timeout:\n${log}`);
            }
            await sleep(500);
        }
    }
    ...
    async tip(): Promise<ChainTip> {
        ...
        const slot = b.slot ?? 0;
        const timeMs = this.slotConfig.zeroTimeMs + slot * this.slotConfig.slotLengthMs;
        return { slot, timeMs, height: b.height ?? 0 };
    }

    /** Poll until the chain's tip time reaches `targetMs` (or throw on timeout). */
    async waitForChainTimeMs(targetMs: number, timeoutSeconds = 60): Promise<void> {
        const deadline = Date.now() + timeoutSeconds * 1000;
        for (;;) {
            if ((await this.tip()).timeMs >= targetMs) return;
            if (Date.now() > deadline) {
                throw new Error(`chain time did not reach ${targetMs}ms within ${timeoutSeconds}s`);
            }
            await sleep(1000);
        }
    }
