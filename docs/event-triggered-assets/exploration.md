...
- **P2 - Event-rule hooks.** Pluggable rules (oracle-driven or contract-driven) that **modify token state, or condition a transfer/lockup/burn, on discrete events** (a time cliff, a market resolution, a maturity, an attestation). On CIP-113 this is a `transfer_logic` or `third_party_logic` withdraw-zero predicate registered in the token's `RegistryNode`, reading event state via reference inputs.
- **P3 - Graduation / unwrap.** A one-way exit that **retires the programmability**, converting the programmable token into a plain native token (or burning it) once the instrument's job is done. Mechanically, the burn is authorized by the substandard's issuance logic (a negative mint under `issuance_mint`), and the graduated asset is minted under a **separate plain native policy**.
...
- **At end of life**, an event-gated P3 transaction burns the CIP-113 token, authorized by the substandard's **issuance logic** withdraw-zero  and mints the equivalent plain native token under a distinct policy (or simply burns, for retire-style cases), removing the contract dependency. A base-layer guarantee keeps this independently auditable: a transaction that spends a registry node can never mint or burn that node's own token, so a graduation tx is always pure issuance, never mixed with a registry reconfiguration.
...
- **CIP-113 programmable-token policy**: the P1 base and the P2 `transfer_logic` / `third_party_logic` predicates.
...
- **Predicates are reactive, not autonomous.** A CIP-113 substandard is up to three withdraw-zero validators registered in the token's `RegistryNode`: issuance logic (mint/burn authority), transfer logic (owner transfers), third-party logic (admin actions). The core `transfer` / `third_party` stake validators *dispatch to* the substandard's withdraw-zero validators on a PLB spend; the issuance logic is a withdraw-zero invoked by `issuance_mint` (a minting policy) on mint/burn. Each handler receives `(redeemer, account, self)` and runs once per transaction, returning approve/reject. There is no scheduler. "Automatically on an event" always means *a keeper detects the event and submits a transaction*, which the predicate then approves. This bounds every P2 claim: the trigger is keeper-driven.
...
- **The third-party path is real and seizure-capable.** A PLB spend with the `SpendViaThirdParty` redeemer (dispatching `ThirdPartyRedeemer`) lets a designated script force transfer / freeze / burn with no owner authorization; seized tokens stay within the base. Freeze is unconditional; only *extraction* is gated and the node's `protected_prefixes` (CIP-67 labels 100/500) shield companion reference NFTs / royalty tokens from seizure and burn. This is the mechanism behind P2's "third-party rules," and any design that holds tokens in a contract is inherently seizure-exposed (Q-RULE-2).
...
- **Registry nodes are live configuration, not frozen records.** A token's `transfer_logic_script`, `third_party_logic_script`, and `global_state_cs` can be re-pointed in place after registration by the frozen issuance credential. Only `minting_logic_script` is frozen for the node's life, and integrators must read the current node and never cache its credentials. This mutability is a holder-trust axis the substandard must take a stance on (Q-RULE-3).
- **Burn-and-remint is the established migration shape.** The substandards documentation's own substandard-swap migration path is exactly *burn the old token + mint the new* (or third-party move + mint), coordinated across transactions.
...
- **Graduation authority & direction (Q-GRAD-2):** P3 removes protections. For a fully-vested token that is safe (retires a defunct kill-switch); pointed the other way, it can strip protections holders relied on. The burn authority is the substandard's issuance credential, which is **frozen at registration**, so the gating must be designed into a mode-aware issuance validator up front (distinguishing Register / UpdateNode / Mint / Burn contexts), not retrofitted. Candidate stances: gate strictly by the same event condition the rules enforce (only once `fully_vested` / `resolved` is on-chain true, so it cannot be a discretionary rug); provide a contract that forces transfer of the new native tokens to the final CIP-113 token holder **and only if the holder signs the tx**; or leave the gating to the developer.
...
- **Seizure posture (Q-RULE-2):** which cases legitimately want the third-party path (regulated recall, dispute resolution) vs must disavow it (holder assurance), and how is the `third_party_logic_script`'s key/governance model disclosed including the extraction-gating choice (issuer allowlist vs script consent) and any `protected_prefixes`?
- **Rule mutability (Q-RULE-3):** the registry lifecycle permits re-pointing `transfer_logic_script` / `third_party_logic_script` retroactively, which could silently replace P1's permissive transfer logic with a gate. Does the substandard disavow re-pointing (static permissive logic), disclose it, or govern it, and how does that stance square with the same frozen issuance credential also gating graduation (Q-GRAD-2)?
...
- **Cardano ecosystem:** CIP-113 (programmable tokens, the base) and its substandard development guide (cardano-foundation/cip113-programmable-tokens, `09-DEVELOPING-SUBSTANDARDS.md`); CIP-68 (metadata, #15), CIP-86 (NFT metadata update oracles) as a P2 pattern; oracle providers Charli3, Orcfax, Pyth as event sources; live conditioning-adjacent precedent in Liqwid qTokens (appreciating-exchange-rate model) and Djed / Indigo (keeper + oracle driven).
- **Internal:** feasibility research on the six "CIP-113 unlocked" use cases (event-triggered = per-UTxO conditioning, feasible, keeper-driven); the #1 / #2 investigations reached the same "CIP-113 adds value at the per-UTxO-conditioning layer" conclusion; the candidate catalog tracker (#16) lists this exploration alongside the sibling candidates.
