...
**Mitigation: tagged outputs.** The own script `OutputReference` can be used to tag the payment output for that swap. Each resolved swap must point to its own output, so two swaps cannot satisfy themselves from a single output.
