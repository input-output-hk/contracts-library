...
`maker` is a `VerificationKeyHash` that identifies who created the swap. `amount`, `policy_id`, and `asset_name` describe the demanded asset and quantity.
...
**Mitigation: tagged outputs.** The own script `OutputReference` can be used to the tag payment output for that swap. Each resolved swap must point to its own output, so two swaps cannot satisfy themselves from a single output.
...
| ---- | -------- | ---------- | ------ |
| Double satisfaction | **High** | Tagged outputs (ยง4.1) | Designed |
...
| Q-DS-1 | Double-satisfaction mitigation confirmed? | Tag payment output with own `OutputReference` | Designed |
